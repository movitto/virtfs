...
module VirtFS::NTFS
  ...
  class IndexRoot
    ...
    def glob_names
      @globNames = glob_entries.collect { |e| e.namespace == NTFS::FileName::NS_DOS ? nil : e.name.downcase }.compact if @globNames.nil?
      @globNames
    end
    ...
    def find_in_entries(name, entries)
      ...
      entries.each do |e|
        if e.last? || name < e.name.downcase
          return e.child? ? findInEntries(name, getIndexAllocEntries(e.child)) : nil
        end
      end
      nil
    end
